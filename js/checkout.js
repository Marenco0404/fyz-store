...
// si el usuario recarga estando en el paso pago, mantenerlo (si existe flag)
    ...
    _bindMetodosPago() {
      ...
      methods.forEach((m) => {
        m.addEventListener("click", async () => {
          ...
          // si seleccionan tarjeta, configurar handler
          if (method === "card") this._initCard();
        });
      });
      
      // Bind card payment button
      const cardPayBtn = document.getElementById("card-pay-btn");
      if (cardPayBtn) {
        cardPayBtn.addEventListener("click", (e) => this._handleCardPayment(e));
      }
    },

    // =================== TARJETA DE CRÉDITO ===================
    _initCard() {
      console.log("✅ Pago con tarjeta inicializado");
      // Aquí se podrían agregar listeners para formatear la tarjeta en tiempo real
    ...
    async _handleCardPayment(e) {
      e.preventDefault();
      e.stopPropagation();

      if (!this._validateFormData()) {
        this._showCheckoutError("❌ Por favor completa la información de envío", "error");
        return;
      }
      ...
      if (!form || !form.checkValidity()) {
        form?.reportValidity?.();
        this._showCheckoutError("Por favor completa todos los campos de la tarjeta", "error");
        return;
      }
      ...
      if (!cardName || cardName.length < 3) {
        this._showCheckoutError("El nombre en la tarjeta debe tener al menos 3 caracteres", "error");
        return;
      }

      if (!cardNumber || cardNumber.length < 13) {
        this._showCheckoutError("Número de tarjeta inválido", "error");
        return;
      }

      if (!expMonth || !expYear || expMonth.length !== 2 || expYear.length !== 4) {
        this._showCheckoutError("Fecha de vencimiento inválida (MM/YYYY)", "error");
        return;
      }

      if (!cvv || cvv.length < 3) {
        this._showCheckoutError("CVV inválido", "error");
        return;
      }
      ...
      if (expYearNum < currentYear || (expYearNum === currentYear && expMonthNum < currentMonth)) {
        this._showCheckoutError("La tarjeta está expirada", "error");
        return;
      }

      try {
        this._showCheckoutError("⏳ Procesando pago con tarjeta...", "info");
        ...
        const pedidoId = await this._postPago({
          metodo: "tarjeta",
          idTransaccion: this._generateTransactionId(),
          totalCRC: total,
          totalUSD: crcToUsd(total),
          fxRate: getFxRate(),
          cardLast4: cardNumber.slice(-4)
        });

        console.log("✅ Pedido con tarjeta registrado:", pedidoId);
        this._showCheckoutError("✅ ¡Pago confirmado con tarjeta! Redirigiendo...", "success");
        ...
        this._showCheckoutError(
          `<strong>⚠️ Error al procesar la tarjeta</strong><br><small style="display: block; margin-top: 0.5rem;">Error: ${error.message}</small>`,
          "error"
        );
      }
    },
